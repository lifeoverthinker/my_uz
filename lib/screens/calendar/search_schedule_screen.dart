...
/// Full-screen search for group/teacher plans — clean MD3-like look,
/// uses MyUz.search_sm prefix icon and MyUz.heart for favorite toggle.
...
class _SearchScheduleScreenState extends State<SearchScheduleScreen> {
  ...
  void _onQueryChanged(String q) {
    _debounce?.cancel();
    _debounce = Timer(const Duration(milliseconds: 250), () => _doSearch(q.trim()));
    setState(() {});
  }
  ...
  Future<void> _doSearch(String q) async {
    ...
    try {
      ...
      for (final m in grows) {
        ...
        final code = (m['kod_grupy'] ?? m['kod'] ?? m['code'])?.toString() ?? '';
    ...
    try {
      ...
      for (final m in trows) {
        ...
        out.add({'type': 'teacher', 'id': id, 'title': name.isNotEmpty ? name : id, 'subtitle': ''});
      }
    } catch (_) {}
  ...
  Widget _buildListTile(Map<String, dynamic> item) {
    ...
    return ListTile(
      ...
      trailing: IconButton(
        icon: Icon(MyUz.heart, color: isFav ? Colors.redAccent : const Color(0xFF9E9E9E)),
      ...
      onTap: () async {
        if (item['type'] == 'group') {
          ...
          // Open group preview screen (existing)
          final res = await Navigator.of(context).push<Map<String, dynamic>?>(MaterialPageRoute(
            builder: (_) => GroupScheduleScreen(groupCode: code, groupId: id, groupName: code),
          ));
          if (!mounted) return;
          if (res != null && res['apply'] == true) {
            Navigator.of(context).pop({'type': 'group', 'id': id, 'code': code});
          }
          return;
        }
        if (item['type'] == 'teacher') {
          ...
          final res = await Navigator.of(context).push<Map<String, dynamic>?>(MaterialPageRoute(
            builder: (_) => TeacherScheduleScreen(teacherId: id, teacherName: name),
          ));
          if (!mounted) return;
          if (res != null && res['apply'] == true) {
            Navigator.of(context).pop({'type': 'teacher', 'id': id, 'name': name});
          }
          return;
        }
      },
  ...
  Widget build(BuildContext context) {
    // MD3-like AppBar with inline search field (no purple background)
    return Scaffold(
        ...
        title: SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: _ctrl.text.isNotEmpty ? IconButton(icon: const Icon(Icons.clear), onPressed: () { _ctrl.clear(); _doSearch(''); }) : null,
            ),
          ),
        ),
        centerTitle: false,
        // removed 'Anuluj' button so search hint has more space
        actions: [],
      ),
      body: SafeArea(
        child: _loading
            ...
            : (_results.isEmpty
            ? Center(child: Text(_ctrl.text.isEmpty ? 'Wpisz frazę aby wyszukać' : 'Brak wyników', style: TextStyle(color: Colors.grey.shade600)))
            : ListView.separated(
