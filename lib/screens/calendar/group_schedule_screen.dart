...
/// Ekran planu grupy.
/// - Tryb podglądu: gdy podasz groupId i/lub groupCode (z wyszukiwarki/ulubionych)
/// - Tryb domyślny: bez parametrów – używa grupy z preferencji
class GroupScheduleScreen extends StatefulWidget {
  final String? groupId;   // opcjonalne: id grupy (preferowane)
  final String? groupCode; // opcjonalne: kod grupy (np. INZ-1.2)
  final String? groupName; // opcjonalny label do subtitle

  const GroupScheduleScreen({super.key, this.groupId, this.groupCode, this.groupName});
...
class _GroupScheduleScreenState extends State<GroupScheduleScreen> {
  ...
  void initState() {
    ...
    _previewMode = (widget.groupId != null && widget.groupId!.isNotEmpty) || (widget.groupCode != null && widget.groupCode!.isNotEmpty);
    _init();
  ...
  Future<void> _toggleFavorite() async {
    final key = ClassesRepository.canonicalFavKey(groupId: _groupId ?? widget.groupId, groupCode: _groupCode ?? widget.groupCode);
    if (key.contains(':')) {
  ...
  Future<String?> _getGroupLabel() async {
    if ((_groupCode ?? widget.groupCode) != null) return (_groupCode ?? widget.groupCode);
  ...
  Future<void> _loadClassesForDay() async {
    ...
    try {
      final list = await ClassesRepository.fetchDayWithWeekFallback(
        ...
        subgroups: _previewMode ? const [] : _subgroups,
  ...
  Widget build(BuildContext context) {
    ...
    // Tylko serduszko (ostatnie, po prawej)
